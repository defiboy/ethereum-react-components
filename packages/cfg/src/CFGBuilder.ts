...
export class CFGService {
  ...
  public buildCFGFromBytecode(bytecode: string): CFGContract {
    const contract: DisassembledContract = this.disassembler.disassembleContract(
      bytecode
    );
    return this.buildCfgContract(contract);
  }

  public completeCFGWithTrace(blocks: CFGBlocks, trace: DebugTrace) {
  ...
  public checkTraceLoops(blocks: CFGBlocks, trace: DebugTrace) {
    ...
    for (const repeatedOffset of repeated) {
      ...
      if (block) {
        const operation = block.operations.find(
          op => op.offset === parseInt(repeatedOffset.offset, 10)
        );
        if (operation) {
  ...
  private calculateCfgBlocks(ops: Operation[]): CFGBlocks {
    ...
    executor.run(0);
    console.log(
      "Calculated dynamic CFG, checking if there are orphan nodes not inspected..."
    );
    executor.runOrphans();
    console.log(
      `Orphan blocks analyzed, tried to complete CFG inspecting bytecode`
    );
